...
function startGame(): void {
  ...
  let wordIndex: number = Math.floor(Math.random() * 100);
  // mysteryWord: the word chosen from cryptidCampWords using wordIndex
  let mysteryWord: string = cryptidCampWords[wordIndex];
  // wordLength: the length of mysteryWord
  let wordLength: number = mysteryWord.length;
  ...
  let mysteryArray: string[] = mysteryWord.split("");
  ...
  const letterInput = document.getElementById("letterInput") as HTMLInputElement | null;
  if (letterInput) {
    ...
    letterInput.addEventListener("keyup", (event: KeyboardEvent) => {
  ...
  function revealLetters(guess: string, wordArray: string[]): void {
    ...
    wordArray.forEach((char, index) => {
      if (char.toLowerCase() === guess) {
        ...
        const box = gameBoard?.querySelector(`div[data-index='${index}']`);
        if (box) {
